...
def doGrid_all():
    ...
    assembler = VectorAssembler(
        inputCols=["day","day_of_week", "hour", "is_airport","is_manhattan","minute", 'pickup_lat_slot', 'pickup_long_slot', "time_of_day_code", "week"],
        outputCol='features')
    print('after')
    output_training = assembler.transform(train)
    output_testing = assembler.transform(test)
    ...
    for x in range(hor):
        print('grid hor:', x)
        for y in range(vert):
            final_data_testing.show()
            final_data_testing_grid = final_data_testing.filter(final_data_testing.pickup_long_slot == vert & final_data_testing.pickup_lat_slot == hor)
            print(type(final_data_testing_grid))
            predictionsDT = dt_model.transform(final_data_testing_grid)
            predictionsLR = lr_model.evaluate(final_data_testing_grid)
