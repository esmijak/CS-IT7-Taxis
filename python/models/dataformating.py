...
def getFeatures() :
    ...
    final_data_training = np.zeros(cst.N_OF_CLUSTERS)
    final_data_testing = np.zeros(cst.N_OF_CLUSTERS)


    for curCluster in range (cst.N_OF_CLUSTERS):
    #for curCluster in cc: to test a specific cluster
        ...
        rows_testing = []
        demandCount = 0
        TOTAL_SLOTS_FOR_LOOP = cst.N_OF_TIME_SLOTS_TEST + cst.N_OF_TIME_SLOTS_TRAIN
        ...
        for curSlotNb in range (TOTAL_SLOTS_FOR_LOOP) :
            ...
            if (len(rows_training) < cst.N_OF_TIME_SLOTS_TRAIN):
